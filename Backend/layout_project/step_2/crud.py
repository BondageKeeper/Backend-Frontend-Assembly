...
DATABASE_URL = 'postgresql+asyncpg://postgres:[WRITE HERE YOUR PASSWORD]@127.0.0.1:5432/PC_customers_v5'
...
async def count_popular_models(session: AsyncSession):
    request = (select(
        BookingInfoDB.computer_model,
        func.count(BookingInfoDB.computer_model).label('computers_popularity')
    ).join(UserInfoDB).group_by(BookingInfoDB.computer_model).order_by(func.count(BookingInfoDB.computer_model).desc()))
    result = await session.execute(request)
    total_value = result.all()
    return [
        {"model_name" : row.computer_model,
